  ...
  return (
    <div className="max-w-7xl mx-auto px-6">
      <div className="text-center mb-20">
        <h2 className="text-cyan-300 font-black uppercase tracking-[0.4em] text-xs mb-4">Investment</h2>
        <h3 className="text-4xl md:text-5xl font-extrabold italic font-serif">
          Best <span className="text-gradient">Pricing Plan</span>
        </h3>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 items-stretch">
        {plans.map((plan, idx) => (
          <div key={idx} className={`relative glass rounded-[2.5rem] flex flex-col border-2 transition-all duration-700 hover:translate-y-[-16px] ${plan.popular ? 'border-cyan-300 scale-105 z-10 shadow-[0_30px_60px_rgba(251,191,36,0.1)]' : 'border-white/5 hover:border-white/20'}`}>
            {plan.popular && (
            ...
            <div className={`p-10 rounded-t-[2.5rem] text-center transition-colors duration-500 ${plan.popular ? 'bg-cyan-300/10' : 'bg-white/[0.02]'}`}>
              <h4 className={`text-sm font-black uppercase tracking-[0.3em] mb-4 ${plan.popular ? 'text-cyan-300' : 'text-gray-400'}`}>{plan.name}</h4>
              ...
              <div className="space-y-5">
                {plan.features.map((feature, fIdx) => (
                  <div key={fIdx} className="flex items-center text-sm font-medium text-gray-400 group cursor-default">
                    <span className="w-5 h-5 rounded-full bg-cyan-300/10 flex items-center justify-center text-cyan-300 text-[10px] mr-4 shrink-0 group-hover:scale-125 transition-transform duration-300">✓</span>
